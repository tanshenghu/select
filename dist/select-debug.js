 ...
 */
define("widget/select/1.0.0/select-debug", [ "$-debug" ], function(require, exports, module) {
    var $ = require("$-debug");
    ...
    select.prototype = {
        ...
        curNode: null,
        eleInit: function() {
            var This = this, bindEvt = function(ui_select) {
                ui_select.on(This.options.eventType || "click", function() {
                    var $this = $(this);
                    if ($this.hasClass("disabled")) return;
                    var originSelect = this.select, key = $this.find(".civalue").val(), ohtml = "";
                    ...
                    This.dropbox.html(ohtml).css({
                        width: This.options.infiniteWidth === false ? $this.outerWidth(true) : undefined,
                        "min-width": $this.outerWidth(true),
                        left: $this.offset().left,
                        top: $this.offset().top + ui_select.outerHeight(true) + ~~This.options.top
                    });
            ...
            if (this.options.trigger.prop("nodeName") === "SELECT") {
                this.dropbox = $("#ui-selectdrop").length ? $("#ui-selectdrop") : $("<div/>", {
                    id: "ui-selectdrop",
                ...
                this.options.trigger.each(function(i, ele) {
                    ele = $(ele);
                    var name = ele.attr("ui-name"), isDisabled = ele.is(":disabled"), width = ele.css("width") ? ele.css("width") : getComputedStyle(ele)["width"] || "132px", unit = width.match(/(\d+)(\w+|%)/), ui_select = $('<span class="ui-select' + (isDisabled ? " disabled" : "") + '" style="width:' + (~~unit[1] <= 22 ? 132 : unit[1]) + unit[2] + '"><input class="ciname" type="text"' + (This.options.readonly === true || isDisabled ? ' readonly="readonly" onfocus="blur()"' : "") + ' value="' + ele.find(":selected").text() + '" title="' + ele.find(":selected").text() + '" style="width:' + ((~~unit[1] <= 22 ? 132 : unit[1]) - 22) + unit[2] + '"><input value="civalue" type="hidden"' + (name ? ' name="' + name + '"' : "") + ' value="' + ele.val() + '"><i></i></span>');
